...
kubelet은 각각의 노드에서 실행되는 주요 "노드 에이전트" 이다.
호스트 네임, 호스트 이름을 오버라이드하는 플래그 또는
클라우드 제공자를 위한 특정 로직 중 하나를 사용하여 노드를 API서버에 등록할 수 있다.

kubelet은 파드 스펙의 측면에서 동작한다.
파드 스펙은 파드를 설명하는 YAML 또는 JSON 오브젝트이다.
kubelet은 다양한 메커니즘(주로 API서버를 통한)을 통해 제공되는 파드 스펙 세트를 사용하고
파드 스펙에 설명된 컨테이너가 동작하고 정상인지 확인한다.
kubelet은 쿠버네티스가 만들지 않은 컨테이너를 관리하지 않는다.

API 서버로부터의 파드 스펙 이외의,
컨테이너 매니페스트를 kubelet에 제공하는 방법으로 세 가지 방법이 있다.

- File: 커맨드 라인 플래그로 전달된 경로.
  해당 경로 아래의 파일들은 업데이트를 위해 주기적으로 모니터링 된다.
  모니터링 주기는 기본적으로 20초이며, 플래그를 통해 설정할 수 있다.
- HTTP 엔드포인트: 커맨드 라인 파라미터로 전달된 HTTP 엔드포인트.
  20초마다 엔드포인트를 확인한다. (또한 플래그로 설정할 수 있다).
- HTTP 서버: kubelet은 HTTP를 수신하고
  간단한 API(현재 낮은 사양)에 응답하여 새로운 매니패스트를 제출할 수 있다.
...
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">true인 경우, 로그 메시지의 헤더에 파일 디렉토리를 추가한다. (사용 중단: 향후 릴리즈에서 제거될 것이다. <a href="https://github.com/kubernetes/enhancements/tree/master/keps/sig-instrumentation/2845-deprecate-klog-specific-flags-in-k8s-components">여기</a>를 참고한다.)</td>
</tr>

<tr>
<td colspan="2">--address string&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;기본값: 0.0.0.0 </td>
</tr>
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">kubelet 서비스를 위한 IP 주소 (모든 인터페이스와 IP families를 수신하기 위해서는 <code>0.0.0.0</code> 또는 <code>::</code>로 설정한다.)  (사용 중단: 이 파라미터는 kubelet 환경 설정 파일의 <code>--config</code> 플래그 명시를 통해 설정될 예정이다. 더 많은 정보는 <a href="https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/">kubectl 환경 설정 파일</a>을 참조한다.)</td>
</tr>
...
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">위험한 sysctl 또는 위험한 sysctl 패턴의 쉼표로 구분된 화이트리스트는 (<code>&ast;</code>로 끝나는) 자신의 책임하에 사용한다. (사용 중단: 이 파라미터는 kubelet 환경 설정 파일의 <code>--config</code> 플래그 명시를 통해 설정될 예정이다. 더 많은 정보는 <a href="https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/">kubectl 환경 설정 파일</a>을 참조한다.)</td>
</tr>
...
<tr>
<td></td><td style="line-height: 130%; word-wrap: break-word;">표준 에러를 파일에 기록한다. (사용 중단: 향후 릴리즈에서 제거될 것이다. <a href="https://github.com/kubernetes/enhancements/tree/master/keps/sig-instrumentation/2845-deprecate-klog-specific-flags-in-k8s-components">여기</a>를 참조한다.)</td>
</tr>
