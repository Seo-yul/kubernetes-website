...
{{< caution >}}
쿠버네티스 클라이언트 구성에 암호를 저장하는 것은 위험하다. 자격 증명 플러그인을 사용하여 별도로 저장하는 것은 더 나은 대안이다.
참조: [client-go 자격증명 플러그인](/docs/reference/access-authn-authz/authentication/#client-go-credential-plugins)
{{< /caution >}}
...
주체 속성을 확인하기 위한 'kubectl' 알파 하위 명령이 있다. `kubectl alpha auth whoami`

[클라이언트의 인증 정보에 대한 API 액세스](/docs/reference/access-authn-authz/authentication/#self-subject-review)
에 대해 더 자세한 내용 읽어보기
